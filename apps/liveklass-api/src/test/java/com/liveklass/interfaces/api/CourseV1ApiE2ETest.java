...
public class CourseV1ApiE2ETest {
	...
	/**
	 * - 강의 목록 조회에 성공할 경우, 강의 목록을 응답으로 반환한다.
	 * - 상태 필터가 주어질 경우, 해당 상태의 강의 목록만 응답으로 반환한다.
	 */
	@DisplayName("GET /api/v1/courses")
	@Nested
	class GetCourses {
		@DisplayName("강의 목록 조회에 성공할 경우, 강의 목록을 응답으로 반환한다.")
		@Test
		public void 강의_목록_조회에_성공할_경우_강의_목록을_응답으로_반환한다() {
			//given
			UserEntity creator = saveUser("creator1", "CREATOR");
			courseRepository.save(CourseEntity.from(createCommand(creator.getId(), "자바 기초")));
			courseRepository.save(CourseEntity.from(createCommand(creator.getId(), "스프링 기초")));

			ParameterizedTypeReference<ApiResponse<List<CourseV1Dto.CourseResponse>>> responseType = new ParameterizedTypeReference<>() {};

			//when
			ResponseEntity<ApiResponse<List<CourseV1Dto.CourseResponse>>> response = testRestTemplate.exchange(
					ENDPOINT,
					HttpMethod.GET,
					HttpEntity.EMPTY,
					responseType
			...
			Assertions.assertAll(
					...
					() -> assertThat(response.getBody().data())
							.extracting(CourseV1Dto.CourseResponse::title)
							.containsExactlyInAnyOrder("자바 기초", "스프링 기초")
			);
		}

		@DisplayName("상태 필터가 주어질 경우, 해당 상태의 강의 목록만 응답으로 반환한다.")
		@Test
		public void 상태_필터가_주어질_경우_해당_상태의_강의_목록만_응답으로_반환한다() {
			//given
			UserEntity creator = saveUser("creator1", "CREATOR");
			...
			Assertions.assertAll(
					...
					() -> assertThat(response.getBody().data().getFirst().title()).isEqualTo("스프링 기초"),
					() -> assertThat(response.getBody().data().getFirst().status()).isEqualTo(CourseStatus.OPEN.name())
			);
		}
	}
